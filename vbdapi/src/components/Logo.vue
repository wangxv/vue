<template>
  <div class="my-index">
    <div class="logo-file">
      ...
      <img v-bind:src="my_url" alt="" class="logo-img">
      <button type="button" class="uploading" v-on:click="upLoading">检验</button>
    </div>
   
    <div>
      <ul class="logo-list">
        <li v-for="item in logoData" class="logo-item">
          <div class="logo-name"><span>名称：</span>
            <span>{{item.name}}</span></div>
        
          <div class="logo-score"><span>可信度：</span><span>{{item.probability | NumFormat}}</span></div>
        </li>
      </ul>
    </div>
  </div>
  ...
  methods: {
    ...
    upLoading() {
      ...
      that.$http.get("/api/oauth/2.0/token", {
        ...
        that.$http.post("/api/rest/2.0/image-classify/v2/logo?access_token=" + response.body.access_token, 'image=' + url
